...
import make_map
import destroy_map
import Stage_2_map_4
import Stage_2_map_9
import Stage_2_map_11
import Stage_2_map_16
...
def enter():
    make_map.make_Map('d:/2DGP/Project/Stage/stage_2/stage_10.txt')
...
def update():
    ...
    if len(game_world.objects[server.Mob_num]) == 0:
        server.Floor_2[10] = True
    if server.isaac_head.nowPos == 4:
        game_framework.change_state(Stage_2_map_4)
    if server.isaac_head.nowPos == 9:
        game_framework.change_state(Stage_2_map_9)
    if server.isaac_head.nowPos == 11:
        game_framework.change_state(Stage_2_map_11)
    if server.isaac_head.nowPos == 16:
        game_framework.change_state(Stage_2_map_16)
