...
import make_map
import destroy_map
import Stage_2_map_18
import Stage_2_map_20
...
def enter():
    if not server.Floor_2[server.isaac_head.nowPos]:
        ...
        if rand_num == 0:
            ...
            game_world.add_objects(server.red_spiders, server.Mob_num)
    make_map.make_Map('d:/2DGP/Project/Stage/stage_2/stage_19.txt')
...
def update():
    ...
    if len(game_world.objects[server.Mob_num]) == 0:
        server.Floor_2[19] = True
    if server.isaac_head.nowPos == 18:
        game_framework.change_state(Stage_2_map_18)
    if server.isaac_head.nowPos == 20:
        game_framework.change_state(Stage_2_map_20)
